...
int cin_fp_on(struct cin_port* cp){	
	...
	fprintf(stdout,"Powering ON CIN Front Panel Boards ........\n");
...
int cin_fp_off(struct cin_port* cp){
	...
	fprintf(stdout,"Powering OFF CIN Front Panel Boards ........\n");
...
int cin_load_firmware(struct cin_port* cp,struct cin_port* dcp,char *filename){
  ...
  int _status; 
	...
	fprintf(stdout,"  CIN FPGA configuration loaded!!\n"); 
	fclose(file);
	return _status;
...
int cin_get_fclk_status(struct cin_port* cp){ 
	...
	if((_val1 == 0xFFFF) || (_val2 == 0xFFFF)){
		...
		fprintf(stdout,"  Unknown FCLK Frequency\n\n"); 
		return -1;
	}	
}
